...
struct KernelRecipe{D <: ContinuousUnivariateDistribution, R <: SemiMetric, F <: AbstractTransform} <: PosteriorApproximator{D}
    d::Type{D}
    ρ::R
    T::F
end
...
distance(K::KernelRecipe, mdl::ImplicitPosterior, x) = K.ρ(K.T(x), K.T(mdl.observed_summs))
KernelRecipe(;k::Type{<:ContinuousUnivariateDistribution} = Uniform, ρ::SemiMetric = euclidean, T = IdentityTransform()) = KernelRecipe(k, ρ, T)
...
struct Kernel{D <: ContinuousUnivariateDistribution, E <: Number, R <: SemiMetric, F <: AbstractTransform, O} <: PosteriorApproximator{D}
    d::Type{D}
    ϵ::E
    ρ::R
    T::F
...
Kernel(summs, ϵ; k::Type{<:ContinuousUnivariateDistribution} = Uniform, ρ::SemiMetric = euclidean, T = IdentityTransform()) = Kernel(k, float(ϵ), ρ, T, summs)
Kernel(m::ImplicitPosterior, ϵ; k::Type{<:ContinuousUnivariateDistribution} = Uniform, ρ::SemiMetric = euclidean, T = IdentityTransform()) = Kernel(k, float(ϵ), ρ, T, m.observed_summs)
...
function pa_length(K::Kernel)
    pas = similar(K.obs, Float64)
    pas .= pa_length(K.T, K.ϵ)
    return pas
end
...
pa_length(PK::ProductKernel) = accumulate(min, transpose(hcat(pa_length.(PK.ks)...)), dims=1)

# Default to the most recently appended kernel.
distance(K::ProductKernel, x) = distance(last(K.ks), x)

(K::ProductKernel)(x) = error("Exact evaluation of pdf of product kernel not possible, use proportional density.")
(K::ProductKernel)(x, ::Val{:logpdf}) = error("Exact evaluation of logpdf of product kernel not possible, use proportional density.")
