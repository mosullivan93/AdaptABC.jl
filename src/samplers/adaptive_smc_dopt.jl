function choose_weights_dfo_dopt(post::ImplicitPosterior{M, P, S} where {M, P}, K::KernelRecipe{Uniform}, th::AbstractMatrix, xs::AbstractMatrix, N_keep::Integer) where {S}
    M = Manifolds.Sphere(Val(S - 1))
    adaptive_estimator = AdaptiveKernelEstimator(SubsetSampleDOPT, post, th, xs, K, N_keep)
...
adaptive_smc_sampler_dopt(post, K::KernelRecipe, N::Integer; drop::Percentage=%(50), R₀=10, p_thresh=0.05, c=0.05) = adaptive_smc_sampler_dopt(post, K, N, drop, R₀, p_thresh, c, k)

function adaptive_smc_sampler_dopt(post::ImplicitPosterior{M, P, S}, K::KernelRecipe{Uniform, D, T}, N::Int,
                                   drop::Percentage=%(50), R₀::Int=10, p_thresh::Float64=0.05, c::Float64=0.05) where {M, P, S, D, T}
    # Setup
    π = prior(post)
    ...
    while true
        if final_iter
            ...
            _, curr_K, mask_idx = choose_weights_dfo_dopt(post, curr_K.recipe, θ, X, N_keep)
