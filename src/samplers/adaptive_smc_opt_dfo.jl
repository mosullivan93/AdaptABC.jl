function choose_weights_nm(mdl::ImplicitPosterior, K::KernelRecipe{Uniform}, th::AbstractMatrix, xs::AbstractMatrix, N_keep::Integer, k::Integer=5)
    d, N = size(th)
    Nx = length(mdl)
    M = Sphere(Nx - 1)
    ...
    function compute_bc(ws)
        ...
        L = logsumexp((log.(N_keep .- ndups) .+ lQs[keep_idx] .+ lNs)/2)
        return L, ind_K, eps_ind, keep_idx
    end
    obj(::AbstractManifold, x) = first(compute_bc(x))

    # mopt_res = NelderMead(M, obj)
    mopt_res = particle_swarm(M, obj)
...
function adaptive_smc_sampler_opt_dfo(mdl::ImplicitPosterior{M}, K::KernelRecipe{Uniform}, N::Integer;
    ...
    while true
        ...
        # Exit early so we can benchmark.
        break
    end
